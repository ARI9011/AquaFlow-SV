...
const StatCard = ({ icon, value, label, sub, colorClass }: any) => (
    <div className="bg-aqua-card p-4 md:p-6 rounded-3xl border border-white/5 relative overflow-hidden group hover:border-white/10 transition-all shadow-lg">
        <div className={`absolute top-0 left-0 w-1 h-full ${colorClass}`} />
        <div className="flex items-start gap-3 md:gap-4">
            <div className="text-xl md:text-2xl group-hover:scale-110 transition-transform">{icon}</div>
            <div className="min-w-0">
                <div className="text-2xl md:text-3xl font-black text-white leading-none">{value}</div>
                <div className="text-[9px] md:text-[10px] font-black text-gray-500 uppercase tracking-widest mt-2">{label}</div>
                <div className="text-[9px] md:text-[10px] text-gray-600 mt-1 italic truncate">{sub}</div>
            </div>
        </div>
    </div>
);
...
export default function Dashboard() {
    return (
        <div className="space-y-8 animate-in fade-in slide-in-from-bottom-4 duration-700">
            ...
            <div className="bg-gradient-to-r from-aqua-cyan/20 via-aqua-cyan/5 to-transparent p-4 md:p-8 lg:p-10 rounded-[2.5rem] border border-aqua-cyan/10 relative overflow-hidden">
                ...
                <div className="relative z-10 flex flex-col md:flex-row justify-between items-start md:items-center gap-4 md:gap-6">
                    <div className="min-w-0">
                        <h2 className="text-2xl md:text-3xl lg:text-4xl font-black tracking-tighter mb-2"> AquaFlow <span className="text-aqua-cyan">SV</span></h2>
                        <p className="text-gray-400 font-medium max-w-md text-xs md:text-sm">
                    ...
                    <div className="relative z-20 bg-aqua-card/80 backdrop-blur-xl px-4 md:px-6 py-3 md:py-4 rounded-2xl border border-aqua-cyan/30 shadow-[0_0_20px_rgba(0,242,234,0.1)] flex-shrink-0">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-3 md:gap-6">
                ...
                <StatCard icon="" value="1" label="Incidencias" sub="Requiere revisión" colorClass="bg-red-500" />
            </div>

            {/* TABLA DE ESTADO POR ZONAS */}
            <div className="bg-aqua-card rounded-[2.5rem] border border-white/5 overflow-hidden shadow-2xl">
                <div className="p-4 md:p-6 lg:p-8 border-b border-white/5 flex flex-col md:flex-row justify-between items-start md:items-center gap-3 bg-white/[0.01]">
                    <h3 className="font-bold text-lg md:text-xl tracking-tight"> Estado por Zonas</h3>
                    <span className="text-[10px] bg-white/5 px-3 py-1 rounded-full text-gray-400 font-black uppercase tracking-widest whitespace-nowrap">
                        Actualizado: Recién
                    </span>
                </div>
                ...
                <div className="overflow-x-auto">
                    <table className="w-full text-left border-collapse min-w-full">
                        <thead className="bg-white/[0.02] text-gray-500 text-[10px] uppercase font-black tracking-[0.15em]">
                            <tr>
                                <th className="px-4 md:px-6 lg:px-8 py-4 md:py-5">Zona / Sector</th>
                                <th className="px-4 md:px-6 lg:px-8 py-4 md:py-5 text-center">Presión</th>
                                <th className="px-4 md:px-6 lg:px-8 py-4 md:py-5 text-center">Flujo</th>
                                <th className="px-4 md:px-6 lg:px-8 py-4 md:py-5 text-center">Estado</th>
                            </tr>
                        </thead>
                        <tbody className="divide-y divide-white/[0.03]">
                            {zonasData.map((zona) => (
                                <tr key={zona.id} className="hover:bg-white/[0.02] transition-colors group">
                                    ...
                                    <td className="px-4 md:px-6 lg:px-8 py-4 md:py-6 text-center font-mono font-bold text-aqua-cyan text-sm">
                                        {zona.presion} <span className="text-[10px] text-gray-600">PSI</span>
                                    </td>
                                    <td className="px-4 md:px-6 lg:px-8 py-4 md:py-6 text-center font-mono font-bold text-gray-300 text-sm">
                                        {zona.flujo} <span className="text-[10px] text-gray-600">L/m</span>
                                    </td>
                                    <td className="px-4 md:px-6 lg:px-8 py-4 md:py-6 text-center">
